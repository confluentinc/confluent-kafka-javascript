...
void Dispatcher::Activate() {
  if (!async) {
    ...
    uv_async_init(uv_default_loop(), async, Dispatcher::func);
...
void Dispatcher::Dispatch(const int _argc, Napi::Value _argv[]) {
  ...
  std::vector<napi_value> args(_argc);
  for (int j = 0; j < _argc; j++) {
    args[j] = _argv[j];
  }

  for (size_t i=0; i < callbacks.size(); i++) {
    callbacks[i]->Value().Call(args.size(), args.data());
  }
}

void Dispatcher::AddCallback(const Napi::Function &cb) {
  Napi::FunctionReference *value = new Napi::FunctionReference();
  *value = Napi::Persistent(cb);
  callbacks.push_back(value);
}

void Dispatcher::RemoveCallback(const Napi::Function &cb) {
  for (size_t i=0; i < callbacks.size(); i++) {
    if (callbacks[i]->Value() == cb) {
...
void EventDispatcher::Flush() {
  if (callbacks.empty() || events.size() < 1) return;
  Napi::Env env = callbacks[0]->Env();
  ...
  for (size_t i=0; i < _events.size(); i++) {
    ...
    switch (_events[i].type) {
      case RdKafka::Event::EVENT_ERROR:
        ...
        argv[1] = Napi::Error::New(env, _events[i].message.c_str()).Value();
      ...
      case RdKafka::Event::EVENT_LOG:
        ...
        (jsobj).Set(Napi::String::New(env, "name"),
          Napi::String::New(env, this->client_name.c_str()));
...
void DeliveryReportDispatcher::Flush() {
  ...
  for (size_t i = 0; i < events_list.size(); i++) {
    ...
    if (event.is_error) {
      ...
      argv[0] = Napi::Number::New(env, static_cast<int>(event.error_code));
    } else {
      argv[0] = env.Null();
    }
    Napi::Object jsobj(Napi::Object::New(env));
...
void RebalanceDispatcher::Flush() {
  if (callbacks.empty() || m_events.size() < 1) return;
  Napi::Env env = callbacks[0]->Env();
  Napi::HandleScope scope(env);
  ...
  for (size_t i=0; i < events.size(); i++) {
    ...
    if (events[i].err == RdKafka::ERR_NO_ERROR) {
      ...
      argv[0] = Napi::Number::New(env, static_cast<int>(events[i].err));
    }
...
int32_t Partitioner::partitioner_cb(const RdKafka::Topic *topic,
  ...
  for (int i = 0; i < 3; i++) args[i] = argv[i];
  Napi::Value return_value = callback.Value().Call(args.size(), args.data());

  int32_t chosen_partition;
  if (!return_value.IsNumber()) {
    chosen_partition = RdKafka::Topic::PARTITION_UA;
  } else {
    chosen_partition = return_value.As<Napi::Number>().Int32Value();
  }
