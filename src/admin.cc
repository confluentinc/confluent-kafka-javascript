...
AdminClient::~AdminClient() { Disconnect(); }
 ...
 */
rd_kafka_event_t *PollForEvent(rd_kafka_queue_t *topic_rkqu,
  ...
  } while (rd_kafka_event_type(event_response) != event_type && attempts > 0);
...
Baton AdminClient::CreateTopic(rd_kafka_NewTopic_t *topic, int timeout_ms) {
...

  {
    ...
    rd_kafka_event_t *event_response = PollForEvent(
        topic_rkqu, RD_KAFKA_EVENT_CREATETOPICS_RESULT, timeout_ms);
    ...
    const rd_kafka_topic_result_t **restopics =
        rd_kafka_CreateTopics_result_topics(  // NOLINT
            create_topic_results, &created_topic_count);
...
Baton AdminClient::DeleteTopic(rd_kafka_DeleteTopic_t *topic, int timeout_ms) {
...

  {
    ...
    // Poll for an event by type in that queue
    rd_kafka_event_t *event_response = PollForEvent(
        topic_rkqu, RD_KAFKA_EVENT_DELETETOPICS_RESULT, timeout_ms);
    ...
    const rd_kafka_topic_result_t **restopics =
        rd_kafka_DeleteTopics_result_topics(  // NOLINT
            delete_topic_results, &deleted_topic_count);
...
Baton AdminClient::CreatePartitions(rd_kafka_NewPartitions_t *partitions,
                                    int timeout_ms) {
  if (!IsConnected()) {
    return Baton(RdKafka::ERR__STATE);
  }

  {
    ...
    rd_kafka_CreatePartitions(m_client->c_ptr(), &partitions, 1, options,
                              topic_rkqu);

    // Poll for an event by type in that queue
    rd_kafka_event_t *event_response = PollForEvent(
        topic_rkqu, RD_KAFKA_EVENT_CREATEPARTITIONS_RESULT, timeout_ms);
    ...
    const rd_kafka_topic_result_t **restopics =
        rd_kafka_CreatePartitions_result_topics(  // NOLINT
            create_partitions_results, &created_partitions_topic_count);

    for (int i = 0; i < static_cast<int>(created_partitions_topic_count);
         i++) {  // NOLINT
      const rd_kafka_topic_result_t *terr = restopics[i];
      const rd_kafka_resp_err_t errcode = rd_kafka_topic_result_error(terr);
      const char *errmsg = rd_kafka_topic_result_error_string(terr);
...
Baton AdminClient::DeleteRecords(rd_kafka_DeleteRecords_t *delete_records,
                                 size_t delete_records_cnt,
...

  {
    ...
    rd_kafka_resp_err_t err = rd_kafka_AdminOptions_set_operation_timeout(
    ...
    rd_kafka_DeleteRecords(m_client->c_ptr(), delete_records,
                           delete_records_cnt, options, rkqu);
...
NAN_METHOD(AdminClient::NodeCreateTopic) {
  ...
  rd_kafka_NewTopic_t *topic =
      Conversion::Admin::FromV8TopicObject(info[0].As<v8::Object>(), errstr);
...
NAN_METHOD(AdminClient::NodeDeleteTopic) {
  ...
  std::string topic_name =
      Util::FromV8String(Nan::To<v8::String>(info[0]).ToLocalChecked());
...
NAN_METHOD(AdminClient::NodeListGroups) {
  ...
  if (is_match_states_set) {
    ...
    if (match_states_array->Length()) {
      match_states =
          Conversion::Admin::FromV8GroupStateArray(match_states_array);
    }
  }
...
NAN_METHOD(AdminClient::NodeDeleteRecords) {
  ...
  rd_kafka_topic_partition_list_t *partitions =
      TopicPartitionOffsetv8ArrayToTopicPartitionList(delete_records_list);
  ...
  Nan::AsyncQueueWorker(new Workers::AdminClientDeleteRecords(
      callback, client, delete_records, delete_records_list->Length(),
