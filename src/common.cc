...
std::vector<RdKafka::TopicPartition*> FromV8Array(
  ...
  for (uint32_t topic_partition_i = 0;
    ...
    Napi::Value topic_partition_value = topic_partition_list.Get(topic_partition_i);
...
return NULL;
  ...
  if (topic_partition.Has(metadataKey)) {
    ...
    if (metadataValue.IsString()) {
      std::string metadataValueStr = metadataValue.As<Napi::String>().Utf8Value();
...
Napi::Object FromMemberDescription(
  ...
  */
  ...
  Napi::Array topicPartitions =
      Conversion::TopicPartition::ToTopicPartitionV8Array(env, partitions, false);
