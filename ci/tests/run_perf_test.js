...
async function main() {
  ...
  const runProducerConsumerMode = async (mode) => {
    ...
    if (concurrentRun) {
      ...
      if (consumerModeAll || consumerModeEachMessage) {
        allPromises.push(runCommand(`MODE=${mode} MESSAGE_COUNT=${messageCount} GROUPID_MESSAGE=${groupIdEachMessage} node performance-consolidated.js --consumer-each-message ${produceToSecondTopicParam} --consumer`));
      }
      if (consumerModeAll || consumerModeEachBatch) {
        allPromises.push(runCommand(`MODE=${mode} MESSAGE_COUNT=${messageCount} GROUPID_BATCH=${groupIdEachBatch} node performance-consolidated.js --consumer-each-batch ${produceToSecondTopicParam} --consumer`));
      }
