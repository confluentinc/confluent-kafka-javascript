...
/**
 * librdkafka error codes for which a failed connect() is worth retrying.
 * These are transient connection/transport errors; anything else (auth, config,
 * fatal) is treated as terminal so a clear failure is surfaced immediately.
 */
const connectRetriableErrorCodes = new Set([
  error.ErrorCodes.ERR__TRANSPORT,
  error.ErrorCodes.ERR__ALL_BROKERS_DOWN,
  error.ErrorCodes.ERR__TIMED_OUT,
  error.ErrorCodes.ERR__TIMED_OUT_QUEUE,
  error.ErrorCodes.ERR__RESOLVE,
  error.ErrorCodes.ERR__SSL,
]);
...
function isConnectRetriable(err) {
  return !!err && connectRetriableErrorCodes.has(err.code);
}
