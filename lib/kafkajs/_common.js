 ...
 * Mirrors KafkaJS: retry by default, unless the error is a known terminal one.
 * @param {{code?: number}} err an error carrying a librdkafka `code`.
 * @returns {boolean} true if the connect should be retried.
 */
function isConnectRetriable(err) {
  return !!err && !connectFatalErrorCodes.has(err.code);
}
