...
/* A type name for error messages about what a builder handed back. */
function describeValue(value) {
  ...
  if (typeof value === 'object') {
    return value.constructor?.name ? `an instance of ${value.constructor.name}` : 'an object';
  }
  return typeof value;
}

/**
 * Checks what a serde builder handed back and splits it into serde and
 * leftover configuration.
 * @param {string} prop - the configuration property naming the builder, for the error message.
 * @param {*} result - what `build()` returned.
 * @returns {[object, object]} the serde and the leftover configuration.
 * @throws {KafkaJSError} with code ERR__INVALID_ARG if the result is not a `[serde, configuration]` pair.
 * @private
 */
function validateSerdeBuildResult(prop, result) {
  if (!Array.isArray(result) || result.length !== 2) {
    throw new error.KafkaJSError(
      `${prop} must return a [serde, configuration] pair from build(), got ${describeValue(result)}`,
      { code: error.ErrorCodes.ERR__INVALID_ARG });
  }
  const [serde, leftover] = result;
  if (leftover === null || typeof leftover !== 'object' || Array.isArray(leftover)) {
    throw new error.KafkaJSError(
      `${prop} returned ${describeValue(leftover)} as the leftover configuration from build(), expected an object`,
      { code: error.ErrorCodes.ERR__INVALID_ARG });
  }
  return [serde, leftover];
}

/**
 * Combines what the builders handed back into the client configuration.
 ...
 * @param {object} config - the configuration the builders were given.
 * @param {object[]} leftovers - the configurations the builders handed back.
 * @returns {object}
 * @private
 */
function intersectSerdeLeftovers(config, leftovers) {
  if (leftovers.length === 0) {
    return config;
  }
  const last = leftovers[leftovers.length - 1];
  const others = leftovers.slice(0, -1);
  const intersection = {};
  for (const [key, value] of Object.entries(last)) {
    if (others.every((leftover) => Object.hasOwn(leftover, key))) {
...
/**
 * Closes every serde, attempting all of them before rethrowing the first
 * error. Null entries and serdes without a `close` method are skipped.
 * @param {Array<object|null>} serdes
 * @returns {Promise<void>}
 * @private
 */
async function closeSerdes(serdes) {
  let firstError = null;
...
/**
 * Creates the cluster id resolver a client hands its serdes, keeping a single
 * resolution in flight.
 *
 * Every call of `fetch` starts a native `rd_kafka_clusterid` worker that may
 * block a libuv thread pool thread for the whole timeout, and nothing in
 * librdkafka deduplicates them. Concurrent callers of the resolver (both
 * serdes, and every `send()` missing the subject name cache at once) therefore
 * all await the same promise, and share the deadline of the call that started
 * it. The in-flight promise is cleared before it settles, so the outcome is not
 * cached here: a failed resolution is retried by the next caller, and
 * librdkafka caches the id itself once known, answering the next call at once.
 *
 * @param {() => Promise<string>} fetch - resolves the cluster id from the client.
 * @returns {() => Promise<string>}
 * @private
 */
function createClusterIdResolver(fetch) {
  let inFlight = null;
  return () => {
    if (inFlight !== null) {
      return inFlight;
    }
    /* `finally` runs before the callers' own handlers, which are attached to
     * the returned promise, so a caller that resolves again right after
     * settling starts a fresh attempt instead of getting this one back. */
    inFlight = Promise.resolve().then(fetch).finally(() => {
      inFlight = null;
    });
    return inFlight;
  };
...
/**
 * Runs the serde builders of a client configuration.
 *
 * Every builder is handed its own copy of the full client configuration, so
 * a property two serdes share is seen by both and a builder mutating its copy
 * affects nobody else. What the client receives is the intersection of what
 * the builders handed back: a property any builder consumed is filtered out,
 * and what remains holds Kafka properties only. If a builder fails, the
 * serdes built before it are closed (not awaited: the builder error is the
 * one to report) and the error is rethrown.
 *
 * @param {object} config - the client configuration, with the builder properties already removed.
 * @param {Array<{prop: string, builder: object|null|undefined, isKey: boolean}>} specs -
 *        the builders in build order; `prop` names the configuration property for error messages.
 * @returns {{serdes: Array<object|null>, config: object}} the serdes in spec order (null where no
 *          builder was configured) and the configuration left over for the client.
 * @private
 */
function buildSerdes(config, specs) {
  const serdes = [];
  const leftovers = [];
  try {
    for (const { prop, builder, isKey } of specs) {
      if (builder === undefined || builder === null) {
        serdes.push(null);
        continue;
      }
      const [serde, leftover] = validateSerdeBuildResult(prop, builder.build({ ...config }, isKey));
      serdes.push(serde);
      leftovers.push(leftover);
    }
  } catch (err) {
    closeSerdes(serdes).catch(() => {});
    throw err;
  }
  return { serdes, config: intersectSerdeLeftovers(config, leftovers) };
}
