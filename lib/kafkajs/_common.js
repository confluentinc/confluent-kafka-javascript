 ...
 * ERR_UNKNOWN (-1) code. Prefer the first error unless it lacks a specific code
 * and the second one has one.
...
function moreInformativeConnectError(primary, fallback) {
  ...
  if (!fallback)
    return primary;
  if (primary.code === error.ErrorCodes.ERR_UNKNOWN && fallback.code !== error.ErrorCodes.ERR_UNKNOWN)
    return fallback;
  return primary;
}
