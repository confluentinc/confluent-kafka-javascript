...
const CompatibilityErrorMessages = Object.freeze({
  ...
  retryFactorMultiplier: () =>
    "The 'retry.factor' and 'retry.multiplier' are not supported. The backoff growth and jitter are handled internally by librdkafka.",
...
function kafkaJSToRdKafkaConfig(config) {
  ...
  /* KafkaJS uses one `retry` policy for both request retries and reconnection,
   * whereas librdkafka splits these into retry.backoff.* and reconnect.backoff.*.
   * Map the same KafkaJS fields onto the reconnect backoff so the connection
   * retry cadence (which librdkafka drives internally during connect) matches
   * the configured backoff. */
  rdkafkaConfig["reconnect.backoff.max.ms"] = maxRetryTime ?? 30000;
  rdkafkaConfig["reconnect.backoff.ms"] = initialRetryTime ?? 300;
...
/**
 * librdkafka error codes for which a failed connect() should NOT be retried.
 * Following KafkaJS, connect() retries by default; only genuinely terminal
 * errors - authentication, authorization, unsupported/misconfiguration, fatal,
 * and terminal client states - are excluded, so retrying cannot turn a clear
 * failure into a slow one.
 */
const connectFatalErrorCodes = new Set([
  /* Authentication */
  error.ErrorCodes.ERR__AUTHENTICATION,
  error.ErrorCodes.ERR_SASL_AUTHENTICATION_FAILED,
  error.ErrorCodes.ERR_UNSUPPORTED_SASL_MECHANISM,
  error.ErrorCodes.ERR_ILLEGAL_SASL_STATE,
  /* Authorization */
  error.ErrorCodes.ERR_TOPIC_AUTHORIZATION_FAILED,
  error.ErrorCodes.ERR_GROUP_AUTHORIZATION_FAILED,
  error.ErrorCodes.ERR_CLUSTER_AUTHORIZATION_FAILED,
  /* Misconfiguration */
  error.ErrorCodes.ERR__INVALID_ARG,
  error.ErrorCodes.ERR_INVALID_CONFIG,
  error.ErrorCodes.ERR__NOT_CONFIGURED,
  /* Unsupported protocol / feature */
  error.ErrorCodes.ERR__UNSUPPORTED_FEATURE,
  error.ErrorCodes.ERR_UNSUPPORTED_VERSION,
  error.ErrorCodes.ERR__UNKNOWN_PROTOCOL,
  error.ErrorCodes.ERR__NOT_IMPLEMENTED,
  /* Fatal and terminal client states */
  error.ErrorCodes.ERR__FATAL,
  error.ErrorCodes.ERR__STATE,
  error.ErrorCodes.ERR__DESTROY,
  error.ErrorCodes.ERR__DESTROY_BROKER,
]);

/**
 * Whether a librdkafka error encountered during connect() should be retried.
 * Mirrors KafkaJS: retry by default, unless the error is a known terminal one or
 * is flagged fatal by librdkafka (which can happen even for an unlisted code).
 * @param {{code?: number, isFatal?: boolean, fatal?: boolean}} err an error carrying a librdkafka `code`.
 * @returns {boolean} true if the connect should be retried.
 */
function isConnectRetriable(err) {
  if (!err)
    return false;
  if (err.isFatal || err.fatal)
    return false;
  return !connectFatalErrorCodes.has(err.code);
}

/**
 * Picks the more informative of two connect errors. The connect callback error
 * carries the specific librdkafka code (e.g. transport failure), while the
 * `event.error` payload for the same failure can arrive with a generic
 * ERR_UNKNOWN (-1) code, or with no code at all. An error flagged fatal always
 * wins, so the fatal flag is not lost; otherwise prefer the first error unless it
 * lacks a specific code and the second one has one.
 * @param {{code?: number, isFatal?: boolean}|null} primary the preferred error.
 * @param {{code?: number, isFatal?: boolean}|null} fallback the alternative error.
 * @returns {{code?: number, isFatal?: boolean}|null} the more informative error.
 */
function moreInformativeConnectError(primary, fallback) {
  if (!primary)
    return fallback;
  if (!fallback)
    return primary;
  ...
  if (fallback.isFatal)
    return fallback;
  const hasSpecificCode = (e) => e.code !== undefined && e.code !== null && e.code !== error.ErrorCodes.ERR_UNKNOWN;
  if (!hasSpecificCode(primary) && hasSpecificCode(fallback))
    return fallback;
  return primary;
}

/* librdkafka's own default for socket.connection.setup.timeout.ms, used when the
 * config does not set it (e.g. a raw librdkafka config with no kafkaJS block). */
const DEFAULT_CONNECTION_SETUP_TIMEOUT_MS = 30000;

/**
 * Per-attempt metadata-fetch timeout for connect(): one connection-setup cycle
 * plus a small margin. Falls back to the librdkafka default when the config does
 * not carry `socket.connection.setup.timeout.ms` (so raw-config users are not
 * given a NaN/0 timeout).
 * @param {object} rdKafkaConfig the finalized librdkafka config.
 * @returns {number} the metadata timeout in milliseconds.
 */
function connectMetadataTimeout(rdKafkaConfig) {
  const setupTimeout = rdKafkaConfig['socket.connection.setup.timeout.ms'];
  const base = typeof setupTimeout === 'number' && Number.isFinite(setupTimeout)
    ? setupTimeout
    : DEFAULT_CONNECTION_SETUP_TIMEOUT_MS;
  return base + 1000;
}

/**
 * Resolves the number of connect() retries from a KafkaJS `retry` config block.
 * Defaults to the KafkaJS default of 5. The backoff between attempts is handled
 * by librdkafka's internal reconnection (reconnect.backoff.*, mapped from the
 * same retry config), so only the attempt count is needed here.
 * @param {object} [retry] the KafkaJS retry config.
 * @returns {number} the number of retries after the initial attempt.
 */
function connectRetries(retry) {
  const r = retry ?? {};
  return r.retries ?? 5;
}
