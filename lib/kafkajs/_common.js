...
/**
 * Resolves the connect-retry parameters from a KafkaJS `retry` config block.
 * Mirrors KafkaJS defaults: retries=5, initialRetryTime=300, maxRetryTime=30000.
 * The delay grows exponentially (x2) per attempt, capped at maxRetryTime. The
 * `factor`/`multiplier` fields are intentionally not honored here (rejected
 * elsewhere), so the growth uses fixed constants.
 * @param {object} [retry] the KafkaJS retry config.
 * @returns {{retries: number, initialRetryTime: number, maxRetryTime: number}}
 */
function connectRetryParams(retry) {
  const r = retry ?? {};
  return {
    retries: r.retries ?? 5,
    initialRetryTime: r.initialRetryTime ?? 300,
    maxRetryTime: r.maxRetryTime ?? 30000,
  };
}
