...
class Producer {
  ...
  async connect() {
    ...
    const rdKafkaConfig = this.#config();
    let retries = connectRetries(this.#userConfig.kafkaJS && this.#userConfig.kafkaJS.retry);

    /* Retrying connect() for a transactional or idempotent producer would risk
     * re-running transaction initialization across rebuilt clients, so it is not
     * retried - the connect is attempted exactly once, as before. */
    if (Object.hasOwn(rdKafkaConfig, 'transactional.id') || rdKafkaConfig['enable.idempotence']) {
      retries = 0;
    }
