...
class Producer {
  ...
  #connectInternalClientOnce() {
    ...
    const metadataTimeout = rdKafkaConfig['socket.connection.setup.timeout.ms'] + 1000;
  ...
  async #connectWithRetries() {
    ...
    try {
      for (;;) {
        ...
        /* disconnect() may have aborted us while this attempt was in flight. If so,
         * tear down whatever this attempt produced (a live client on a late success,
         * or a failed one) and fail, so the caller never ends up with a client it
         * believes was disconnected. */
        ...
        /* Tear down this attempt's client before retrying or giving up. */
        await this.#teardownFailedClient(this.#internalClient);
  ...
  async disconnect() {
    ...
    /* Connecting: signal the retry loop to abort. It will wake from any backoff,
     * stop retrying, and leave the CONNECTING state. There is no connected client
     * to tear down, so we are done once it settles. */
