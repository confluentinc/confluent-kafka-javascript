...
class Producer {
  ...
  async connect() {
    ...
    const rdKafkaConfig = this.#config();
    let { retries, initialRetryTime, maxRetryTime } = connectRetryParams(this.#userConfig.kafkaJS && this.#userConfig.kafkaJS.retry);
    ...
    let attempt = 0;
    let retryDelay = initialRetryTime;
    try {
      for (;;) {
        try {
          ...
          if (attempt >= retries || !isConnectRetriable(err)) {
          ...
          /* Wait for the backoff, but wake early if disconnect() aborts us. */
          await Timer.withTimeout(retryDelay, this.#connectAbort);
          if (this.#connectAbort.resolved) {
            this.#state = ProducerState.DISCONNECTED;
            throw new error.KafkaJSError("Connect aborted by disconnect().", { code: error.ErrorCodes.ERR__STATE });
          }
          retryDelay = Math.min(retryDelay * 2, maxRetryTime);
          attempt++;
        }
      }
    } finally {
