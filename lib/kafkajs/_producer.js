...
class Producer {
  ...
  #finalizedConfig() {
    ...
    if (Object.hasOwn(rdKafkaConfig, 'js.key.serializer.builder')) {
      this.#keySerializer = rdKafkaConfig['js.key.serializer.builder'].build(rdKafkaConfig, true);
      delete rdKafkaConfig['js.key.serializer.builder'];
    }
    if (Object.hasOwn(rdKafkaConfig, 'js.value.serializer.builder')) {
      this.#valueSerializer = rdKafkaConfig['js.value.serializer.builder'].build(rdKafkaConfig, false);
  ...
  async #readyCb() {
    ...
    const keyNeedsClusterId = this.#keySerializer != null && this.#keySerializer.needsClusterId();
    const valueNeedsClusterId = this.#valueSerializer != null && this.#valueSerializer.needsClusterId();
    if (keyNeedsClusterId || valueNeedsClusterId) {
      ...
      this.clusterId({ timeout: 60000 }).then((result) => {
        if (keyNeedsClusterId)
          this.#keySerializer.setClusterId(result);
        if (valueNeedsClusterId)
          this.#valueSerializer.setClusterId(result);
        // Resolve the promise.
        this.#connectPromiseFunc['resolve']();
      }).catch((err) => {
        /* clusterId() already rejects with a KafkaJS error. */
        this.#connectPromiseFunc['reject'](err);
      });
    } else {
      // Resolve the promise.
