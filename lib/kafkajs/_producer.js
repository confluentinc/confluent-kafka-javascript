...
class Producer {
  ...
  /**
   * Resolves to signal an in-progress connect() retry loop to abort, set while
   * connect() is retrying and fired by disconnect().
   * @type {DeferredPromise|null}
   */
  #connectAbort = null;

  /**
   * The in-flight connect() promise, used to dedupe concurrent connect() calls
   * onto a single connection attempt. Cleared once the connect settles.
   * @type {Promise<void>|null}
   */
  #connectPromise = null;

  /**
   * The [event, listener] pairs this class registered on the current internal
   * client, so a failed attempt's client can be detached from them without
   * removing the base client's own listeners.
   * @type {Array<Array<(string|Function)>>}
   */
  #internalClientListeners = [];
  ...
  async #readyCb() {
    ...
    this.#onInternalClient('delivery-report', this.#deliveryCallback.bind(this));
  ...
  /**
   * Perform a single connection attempt: build the internal client, wire up its
   * event handlers, and connect to the bootstrap brokers.
   *
   * The returned promise resolves via #readyCb once the client is ready, and
   * rejects (with the original librdkafka error) if the connect callback reports
   * an error.
   * @returns {Promise<void>} Resolves when the internal client is ready, rejects on error.
   * @private
   */
  #connectInternalClientOnce() {
    const rdKafkaConfig = this.#config();

    /* Reset to CONNECTING for each attempt: a previous attempt may have advanced
     * the transaction state machine (INITIALIZING_TRANSACTIONS) before failing, and
     * this attempt builds a fresh client that must re-enter #readyCb cleanly. */
    this.#state = ProducerState.CONNECTING;
    this.#connectionError = null;
    this.#internalClientListeners = [];
    this.#internalClient = new RdKafka.Producer(rdKafkaConfig);
    this.#onInternalClient('ready', this.#readyCb.bind(this));
    this.#onInternalClient('event.error', this.#errorCb.bind(this));
    this.#onInternalClient('error', this.#errorCb.bind(this));
    this.#onInternalClient('event.log', (msg) => loggerTrampoline(msg, this.#logger));
    if (this.#statsCb) {
      this.#onInternalClient('event.stats', this.#statsCb.bind(this));
    }

    /* Bound the initial metadata fetch to one connection-setup cycle plus a small
     * margin, rather than the 30s default. librdkafka retries the bootstrap
     * connection internally (paced by reconnect.backoff.*) during this window; the
     * outer connect() loop supplies additional attempts. */
    const metadataTimeout = connectMetadataTimeout(rdKafkaConfig);

    return new Promise((resolve, reject) => {
      this.#connectPromiseFunc = { resolve, reject };
      this.#internalClient.connect({ timeout: metadataTimeout }, (err) => {
        if (err) {
          const rejectionError = moreInformativeConnectError(this.#connectionError, err);
  ...
  /**
   * Registers a listener on the internal client and records it, so it can later
   * be removed on its own by #teardownFailedClient.
   * @param {string} event the event name.
   * @param {Function} listener the listener.
   * @private
   */
  #onInternalClient(event, listener) {
    this.#internalClient.on(event, listener);
    this.#internalClientListeners.push([event, listener]);
  }

  /**
   * Tear down an internal client from a failed connect attempt so it does not
   * leak and cannot fire late events into a subsequent attempt.
   *
   * Our functional handlers are detached, but a no-op `error` handler is kept so
   * a late native `error` emission does not become an unhandled EventEmitter
   * error (which would terminate the process). If the client had already reached
   * `ready` (e.g. transaction initialization failed after connecting), it is a
   * live connected client and is disconnected; the returned promise resolves only
   * once that disconnect completes, so the next attempt does not build a new
   * client (with the same transactional.id) while the old one is still closing.
   * @param {import("../rdkafka").Producer} client the client to tear down.
   * @returns {Promise<void>} resolves once the client has been torn down.
   * @private
   */
  async #teardownFailedClient(client) {
    if (!client) {
      return;
    }
    /* Detach only the handlers this class installed; the base client's own
     * listeners (e.g. the one clearing its connected flag on 'disconnected') must
     * stay so the disconnect below leaves the client in a consistent state. */
    for (const [event, listener] of this.#internalClientListeners) {
      client.removeListener(event, listener);
    }
    this.#internalClientListeners = [];
    /* Keep a safe sink for any late `error` emission. */
    client.on('error', () => {});
   ...
   */
  async connect() {
    /* Dedupe concurrent connect() calls onto a single in-flight attempt. */
  ...
  /**
   * Runs the connect attempts with retries. See {@link connect}.
   * @returns {Promise<void>} Resolves when connected, rejects when all retries fail.
   * @private
   */
  async #connectWithRetries() {
    const retries = connectRetries(this.#userConfig.kafkaJS && this.#userConfig.kafkaJS.retry);

    this.#connectAbort = new DeferredPromise();
    let attempt = 0;
    try {
      for (;;) {
        let attemptError = null;
        try {
          await this.#connectInternalClientOnce();
        } catch (err) {
          attemptError = err;
        }

        /* A successful attempt that was not aborted is the only success path. */
        if (!attemptError && !this.#connectAbort.resolved) {
          return;
        }

        /* Tear down whatever this attempt produced - a live client on a (late)
         * success we are about to abandon, or a failed one - before deciding what
         * to do next. Teardown is awaited so we never build a new client (with the
         * same transactional.id) while the previous one is still closing. */
        await this.#teardownFailedClient(this.#internalClient);

        /* Re-check the abort flag AFTER teardown: a concurrent disconnect() may have
         * fired during the attempt or during the awaited teardown. An abort ends the
         * loop regardless of retries left. */
        if (this.#connectAbort.resolved) {
          this.#state = ProducerState.DISCONNECTED;
          throw new error.KafkaJSError("Connect aborted by disconnect().", { code: error.ErrorCodes.ERR__STATE });
        }

        const kafkaJsError = createKafkaJsErrorFromLibRdKafkaError(attemptError);
        if (attempt >= retries || !isConnectRetriable(attemptError)) {
          this.#state = ProducerState.DISCONNECTED;
          throw kafkaJsError;
        }

        /* No JS-level backoff between attempts: librdkafka already backs off the
         * bootstrap reconnection internally (reconnect.backoff.*) during each
         * attempt's metadata window. */
        this.#logger.warn(`Connection attempt ${attempt + 1} failed, retrying: ${kafkaJsError.message}`, this.#createProducerBindingMessageMetadata());
        attempt++;
      }
    } finally {
  ...
  async disconnect() {
    ...
    /* Connecting: signal the retry loop to abort, then wait for it to settle so
     * that any in-flight attempt is finished and its client torn down before we
     * return. The connect promise rejects with the abort error; that is expected. */
    if (this.#connectAbort) {
      this.#connectAbort.resolve();
      if (this.#connectPromise) {
