    ...
    try {
      for (;;) {
        try {
          ...
          const kafkaJsError = createKafkaJsErrorFromLibRdKafkaError(err);

          if (attempt >= retries || !isConnectRetriable(err) || this.#connectAbort.resolved) {
          ...
          /* The failed internal client has already been torn down internally; drop
           * our listeners on it so a late event can't fire into the next attempt. */
          if (this.#internalClient) {
            this.#internalClient.removeAllListeners();
          }
