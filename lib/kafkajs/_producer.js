...
class Producer {
  ...
  #finalizedConfig() {
     ...
     * throws, the first serializer would otherwise be leaked. */
    try {
      if (keySerializerBuilder !== undefined && keySerializerBuilder !== null) {
        this.#keySerializer = keySerializerBuilder.build(rdKafkaConfig, true);
      }
      if (valueSerializerBuilder !== undefined && valueSerializerBuilder !== null) {
        this.#valueSerializer = valueSerializerBuilder.build(rdKafkaConfig, false);
      }
    } catch (err) {
      /* Not awaited: the builder error is the one to report, and the
       * serializer close only releases resources. */
      this.#closeSerdes().catch(() => {});
      throw err;
    }
  ...
  async #closeSerdes() {
    ...
    let firstError = null;
    for (const serde of serdes) {
      if (typeof serde?.close !== 'function') {
        continue;
      }
      try {
        await serde.close();
