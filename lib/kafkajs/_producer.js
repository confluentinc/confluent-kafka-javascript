...
class Producer {
  ...
  /**
   * Key serializer for the producer, if any.
   * @type {import("../../types/kafkajs").Serializer | null}
   */
  #keySerializer = null;

  /**
   * Value serializer for the producer, if any.
   * @type {import("../../types/kafkajs").Serializer | null}
   */
  ...
  #finalizedConfig() {
    ...
    const keySerializerBuilder = rdKafkaConfig['js.key.serializer.builder'];
    const valueSerializerBuilder = rdKafkaConfig['js.value.serializer.builder'];
    delete rdKafkaConfig['js.key.serializer.builder'];
    delete rdKafkaConfig['js.value.serializer.builder'];

    /* Each builder sees the full configuration and hands back what it did not
     * consume; the producer is created with the intersection of those. The
     * serializers built here are owned by the producer: if the second builder
     * throws, the first serializer is closed rather than leaked. */
    const built = buildSerdes(rdKafkaConfig, [
      { prop: 'js.key.serializer.builder', builder: keySerializerBuilder, isKey: true },
      { prop: 'js.value.serializer.builder', builder: valueSerializerBuilder, isKey: false },
    ]);
    [this.#keySerializer, this.#valueSerializer] = built.serdes;

    return built.config;
  }

  /**
   * Releases the serializers built from the `js.*.serializer.builder`
   * configuration, if any. Idempotent: the serializers are detached before
   * being closed, so a concurrent or repeated call finds nothing to close.
   * Both are attempted even if the first throws; the first error is rethrown.
   * @returns {Promise<void>}
   * @private
   */
  async #closeSerdes() {
    const serdes = [this.#keySerializer, this.#valueSerializer];
    this.#keySerializer = null;
    this.#valueSerializer = null;
    await closeSerdes(serdes);
  }

  /**
   * Hands the serializers a resolver for the id of the connected cluster.
   * The resolver is invoked by the serializer lazily, on first need, so
   * connecting does not wait on a metadata round trip. Both serializers share
   * one resolver, and concurrent invocations share a single call into
   * librdkafka.
   * @private
   */
  #setSerdeClusterIdResolver() {
    // Timeout of 60 seconds corresponds to the default max.block.ms in Java for metadata retrieval.
    const resolver = createClusterIdResolver(() => this.clusterId({ timeout: 60000 }));
    for (const serde of [this.#keySerializer, this.#valueSerializer]) {
      if (typeof serde?.setClusterIdResolver === 'function') {
  ...
  async #readyCb() {
    ...
    this.#setSerdeClusterIdResolver();
  ...
  async connect() {
    ...
    /* Resolve the configuration, which builds the serializers, before leaving
     * INIT: a throwing builder must leave the producer where it was, so that
     * connect() can be retried. */
    const rdKafkaConfig = this.#config();
    ...
    try {
      ...
      this.#state = ProducerState.INIT;
      await this.#closeSerdes().catch(() => {});
      throw err;
    }
    ...
    return new Promise((resolve, reject) => {
      ...
      this.#internalClient.connect(null, (err) => {
        if (err) {
          ...
          /* The producer cannot be used any more: release the serializers it
           * built. The connection error is the one to report. */
          this.#closeSerdes()
            .catch(() => {})
            .then(() => reject(createKafkaJsErrorFromLibRdKafkaError(rejectionError)));
        }
      });
    });
  }
   ...
   *
   * Serializers built from the `js.*.serializer.builder` configuration are
   * closed too, after the client, whether or not the client disconnect
   * succeeded.
  ...
  async disconnect() {
    ...
    try {
      ...
      /* The client is closed first: a flush during disconnect may still
       * serialize. */
      await this.#closeSerdes();
    }
  }
  ...
  async send(sendOptions) {
    ...
    /* Fail fast on an invalid record: serializing (and possibly registering
     * a schema) for a record that cannot be produced is wasted work. */
    if (typeof sendOptions.topic !== 'string' || sendOptions.topic === '') {
      throw new error.KafkaJSError("topic must be a non-empty string", { code: error.ErrorCodes.ERR__INVALID_ARG });
    }
    if (!Array.isArray(sendOptions.messages)) {
      throw new error.KafkaJSError("messages must be an array", { code: error.ErrorCodes.ERR__INVALID_ARG });
    }
    ...
    for (let i = 0; i < sendOptions.messages.length; i++) {
      ...
      /* An absent key or value is equivalent to a null one: it must not be
       * passed to a serializer, and it must not be rejected by the checks
       * below. */
      let key = msg.key === undefined ? null : msg.key;
      let value = msg.value === undefined ? null : msg.value;
      if (this.#keySerializer && key !== null) {
        try {
          key = await this.#keySerializer.serialize(sendOptions.topic, key, msg.headers);
        } catch (err) {
          throw new error.KeySerializationError(err);
        }
      }
      if (this.#valueSerializer && value !== null) {
        try {
          value = await this.#valueSerializer.serialize(sendOptions.topic, value, msg.headers);
        } catch (err) {
          throw new error.ValueSerializationError(err);
        }
      } else if (typeof value === 'string') {
        value = Buffer.from(value);
      }
      if (key !== null && typeof key !== 'string' && !Buffer.isBuffer(key)) {
        throw new error.KafkaJSError("Key must be a string or a Buffer. Check your serializer configuration", { code: error.ErrorCodes.ERR__INVALID_ARG });
      }
      if (value !== null && !Buffer.isBuffer(value)) {
        throw new error.KafkaJSError("Value must be a string or a Buffer. Check your serializer configuration", { code: error.ErrorCodes.ERR__INVALID_ARG });
      }
      ...
      msgPromises.push(new Promise((resolve, reject) => {
        ...
        try {
          this.#internalClient.produce(sendOptions.topic, msg.partition, value, key, msg.timestamp, opaque, msg.headers);
        } catch (err) {
          reject(err);
        }
      }));
  ...
  */
  ...
  /**
   * Get the id of the cluster this producer is connected to.
   *
   * The cluster id is received as part of a metadata response and is cached by
   * librdkafka, so this generally resolves without contacting a broker.
   *
   * @param {object?} options
   * @param {number?} options.timeout - Number of ms to wait for the cluster id to
   *                                    become available. Defaults to 5000. A value
   *                                    of 0 only consults the cache.
   * @returns {Promise<string>} a promise that resolves to the cluster id.
   */
  async clusterId(options = {}) {
    if (this.#state !== ProducerState.CONNECTED) {
      throw new error.KafkaJSError("Cannot fetch the cluster id without awaiting connect()", { code: error.ErrorCodes.ERR__STATE });
    }

    const timeout = Object.hasOwn(options, 'timeout') ? options.timeout : 5000;

    return new Promise((resolve, reject) => {
      this.#internalClient.clusterId(timeout, (err, clusterId) => {
        if (err) {
          reject(createKafkaJsErrorFromLibRdKafkaError(err));
          return;
        }
