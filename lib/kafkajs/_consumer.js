...
class Consumer {
  ...
  #finalizedConfig() {
    ...
    if (Object.hasOwn(rdKafkaConfig, 'js.key.deserializer.builder')) {
      this.#keyDeserializer = rdKafkaConfig['js.key.deserializer.builder'].build(rdKafkaConfig, true);
      delete rdKafkaConfig['js.key.deserializer.builder'];
    }

    if (Object.hasOwn(rdKafkaConfig, 'js.value.deserializer.builder')) {
      this.#valueDeserializer = rdKafkaConfig['js.value.deserializer.builder'].build(rdKafkaConfig, false);
  ...
  #readyCb() {
    ...
    const keyNeedsClusterId = this.#keyDeserializer != null && this.#keyDeserializer.needsClusterId();
    const valueNeedsClusterId = this.#valueDeserializer != null && this.#valueDeserializer.needsClusterId();
    if (keyNeedsClusterId || valueNeedsClusterId) {
      ...
      this.clusterId({ timeout: 60000 }).then((result) => {
        if (keyNeedsClusterId)
          this.#keyDeserializer.setClusterId(result);
        if (valueNeedsClusterId)
          this.#valueDeserializer.setClusterId(result);
        // Resolve the promise.
  ...
  async #messageProcessor(m, config, worker) {
    ...
    if (this.#keyDeserializer || this.#valueDeserializer) {
      if (message.key !== null && this.#keyDeserializer) {
        message.deserializedKey = await this.#keyDeserializer.deserialize(payload.topic, message.key, message.headers);
      }
      if (message.value !== null && this.#valueDeserializer) {
        message.deserializedValue = await this.#valueDeserializer.deserialize(payload.topic, message.value, message.headers);
      }
    }
  ...
  async #batchProcessor(ms, config, worker) {
    ...
    if (this.#keyDeserializer || this.#valueDeserializer) {
      for (const message of payload.batch.messages) {
        if (message.key !== null && this.#keyDeserializer) {
          message.deserializedKey = await this.#keyDeserializer.deserialize(payload.batch.topic, message.key, message.headers);
        }
        if (message.value !== null && this.#valueDeserializer) {
          message.deserializedValue = await this.#valueDeserializer.deserialize(payload.batch.topic, message.value, message.headers);
        }
      }
    }
