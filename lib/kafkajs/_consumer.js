...
class Consumer {
   ...
   * `ready`, it is a live connected client and is disconnected; the returned
   * promise resolves only once that disconnect completes, so the next attempt does
   * not build a new client while the old one is still closing.
   * @param {import("../rdkafka").KafkaConsumer} client the client to tear down.
  ...
  async #connectWithRetries() {
    ...
    try {
      for (;;) {
         ...
         * to do next. Teardown is awaited so we never build a new client while the
         * previous one is still closing. */
        ...
        this.#logger.warn(`Connection attempt ${attempt + 1} failed, retrying: ${kafkaJsError.message}`, this.#createConsumerBindingMessageMetadata());
        attempt++;
      }
    } finally {
