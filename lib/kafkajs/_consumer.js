...
class Consumer {
   ...
   * @type {import("../../types/kafkajs").Deserializer|null}
   */
  ...
  #finalizedConfig() {
    ...
    const keyDeserializerBuilder = rdKafkaConfig['js.key.deserializer.builder'];
    const valueDeserializerBuilder = rdKafkaConfig['js.value.deserializer.builder'];
    delete rdKafkaConfig['js.key.deserializer.builder'];
    delete rdKafkaConfig['js.value.deserializer.builder'];

    /* Each builder sees the full configuration and hands back what it did not
     * consume; the consumer is created with the intersection of those. The
     * deserializers built here are owned by the consumer: if the second
     * builder throws, the first deserializer is closed rather than leaked. */
    const built = buildSerdes(rdKafkaConfig, [
      { prop: 'js.key.deserializer.builder', builder: keyDeserializerBuilder, isKey: true },
      { prop: 'js.value.deserializer.builder', builder: valueDeserializerBuilder, isKey: false },
    ]);
    [this.#keyDeserializer, this.#valueDeserializer] = built.serdes;

    return built.config;
  }

  /**
   * Releases the deserializers built from the `js.*.deserializer.builder`
   * configuration, if any. Idempotent: the deserializers are detached before
   ...
   */
  async #closeSerdes() {
    const serdes = [this.#keyDeserializer, this.#valueDeserializer];
  ...
  /**
   * Hands the deserializers a resolver for the id of the connected cluster.
   * The resolver is invoked by the deserializer lazily, on first need, so
   * connecting does not wait on a metadata round trip. Both deserializers
   * share one resolver, and concurrent invocations share a single call into
   * librdkafka.
   * @private
   */
  #setSerdeClusterIdResolver() {
    ...
    for (const serde of [this.#keyDeserializer, this.#valueDeserializer]) {
  ...
  #createPayload(message, worker) {
    ...
    return {
      ...
      message: {
        key,
        value: message.value,
        deserializedKey: { key: null, error: null },
        deserializedValue: { value: null, error: null },
        timestamp,
        attributes: 0,
  ...
  async connect() {
    ...
    /* Resolve the configuration, which builds the deserializers, before
     * leaving INIT: a throwing builder must leave the consumer where it was,
     * so that connect() can be retried. */
    ...
    return new Promise((resolve, reject) => {
      ...
      this.#internalClient.connect(null, (err) => {
        if (err) {
          ...
          /* The consumer cannot be used any more: release the deserializers
           * it built. The connection error is the one to report. */
          this.#closeSerdes()
            .catch(() => {})
  ...
  /**
   * Deserializes the key and the value of a message with the configured
   * deserializers, filling in its deserializedKey and deserializedValue.
   *
   * A deserializer that throws does not fail the message: the error is wrapped in a
   * KeyDeserializationError or a ValueDeserializationError and set as the `error` of
   * the corresponding field, so that the record is still delivered and the
   * application can decide what to do with it. Both deserializers are attempted, so
   * one failing does not hide the result of the other.
   *
   * @param {string} topic The topic the message was consumed from.
   * @param {object} message The message to deserialize. Modified in place.
   * @private
   */
  async #deserializeMessage(topic, message) {
    if (message.key !== null && this.#keyDeserializer) {
      try {
        message.deserializedKey.key =
          await this.#keyDeserializer.deserialize(topic, message.key, message.headers);
      } catch (e) {
        message.deserializedKey.error = new error.KeyDeserializationError(e);
      }
    }

    if (message.value !== null && this.#valueDeserializer) {
      try {
        message.deserializedValue.value =
          await this.#valueDeserializer.deserialize(topic, message.value, message.headers);
      } catch (e) {
        message.deserializedValue.error = new error.ValueDeserializationError(e);
      }
    }
  }
  ...
  async #messageProcessor(m, config, worker) {
    ...
    if (this.#keyDeserializer || this.#valueDeserializer) {
      await this.#deserializeMessage(payload.topic, message);
    }
  ...
  async #batchProcessor(ms, config, worker) {
    ...
    if (this.#keyDeserializer || this.#valueDeserializer) {
      for (const message of payload.batch.messages) {
        await this.#deserializeMessage(payload.batch.topic, message);
      }
    }
  ...
  /**
   * Get the id of the cluster this consumer is connected to.
   *
  ...
  async clusterId(options = {}) {
    if (this.#state !== ConsumerState.CONNECTED) {
      throw new error.KafkaJSError('Cluster id can only be fetched while connected.', { code: error.ErrorCodes.ERR__STATE });
    }
  ...
  async disconnect() {
    ...
    try {
      ...
      /* The client is closed first: workers may still be deserializing
       * cached messages while it disconnects. */
