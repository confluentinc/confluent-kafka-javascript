...
class Consumer {
  ...
  /**
   * The asyncLocalStorage for the consumer.
   * Initialized with the asyncLocalStorage from this.#lock.asyncLocalStorage.
   * This makes the #lock reentrant, and allows public API to be called from anywhere
   * as long as we add a lock acquire within the public API.
   */
  #asyncLocalStorage = this.#lock.asyncLocalStorage;
  ...
  async run(config) {
    ...
    this.#asyncLocalStorage.run(1 /* a random truthy constant */, () => this.#runInternal(config));
  }
