...
class Consumer {
  ...
  async #nextFetchRetry() {
    if (this.#fetchInProgress) {
       ...
       * timer expires. We cannot just await the `waiter` promise. This is because
       * it's possible, in the time between the fetch and when `nextFetchRetry` are
       * called, that the queue becomes non-empty. In that case, this worker will
       * halt for no reason. Having a bounded wait ensures that even if we miss
       * being woken up, we will still make progress with this worker soon. */
  ...
  async #checkMaxPollIntervalNotExceeded(now) {
    ...
    this.#maxPollIntervalRestart = replaceDeferredPromise(this.#maxPollIntervalRestart);
