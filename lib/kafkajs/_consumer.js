...
class Consumer {
  ...
  #finalizedConfig() {
     ...
     * builder throws, the first deserializer would otherwise be leaked. */
    try {
      if (keyDeserializerBuilder !== undefined && keyDeserializerBuilder !== null) {
        this.#keyDeserializer = keyDeserializerBuilder.build(rdKafkaConfig, true);
      }
      if (valueDeserializerBuilder !== undefined && valueDeserializerBuilder !== null) {
        this.#valueDeserializer = valueDeserializerBuilder.build(rdKafkaConfig, false);
      }
    } catch (err) {
      /* Not awaited: the builder error is the one to report, and the
       * deserializer close only releases resources. */
