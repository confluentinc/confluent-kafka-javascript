...
class Consumer {
  ...
  #kafkaJSToConsumerConfig(kjsConfig, isClassicProtocol = true) {
    ...
    /* maxBytes bounds the whole Fetch response, so it maps to fetch.max.bytes.
     * It must not be written to fetch.message.max.bytes: that property is an
     * alias of max.partition.fetch.bytes (set from maxBytesPerPartition above),
     * so assigning it here silently overrode the per-partition bound. */
    if (Object.hasOwn(kjsConfig, 'maxBytes')) {
      rdKafkaConfig['fetch.max.bytes'] = kjsConfig.maxBytes;
    } else {
      rdKafkaConfig['fetch.max.bytes'] = 10485760;
    }
