...
class Consumer {
  ...
  #finalizedConfig() {
    ...
    if (rdKafkaConfig['js.consumer.max.cached.batches.per.worker'] !== undefined) {
      const maxCachedBatchesPerWorker = +rdKafkaConfig['js.consumer.max.cached.batches.per.worker'];
      if (!Number.isInteger(maxCachedBatchesPerWorker) || (maxCachedBatchesPerWorker <= 1)) {
        throw new error.KafkaJSError(
          "'js.consumer.max.cached.batches.per.worker' must be a positive integer greater or equal to one.",
          { code: error.ErrorCodes.ERR__INVALID_ARG });
      }
      this.#maxCachedBatchesPerWorker = maxCachedBatchesPerWorker;
      delete rdKafkaConfig['js.consumer.max.cached.batches.per.worker'];
    }
  ...
  #updateMaxMessageCacheSize() {
    ...
    if (this.#lastFetchedMessageCnt > 0 && this.#lastFetchClockNs > 0n &&
      ...
      if (this.#messageCacheMaxSize < this.#maxBatchesSize)
        // Keep at least one batch or one message per worker.
        // It's possible less workers than requested were active in previous run.
        this.#messageCacheMaxSize = this.#maxBatchesSize;
      else if (this.#messageCacheMaxSize > this.#maxBatchesSize * this.#maxCachedBatchesPerWorker)
        // Keep at most `#maxCachedBatchesPerWorker` messages or batches per requested worker.
        this.#messageCacheMaxSize = this.#maxBatchesSize * this.#maxCachedBatchesPerWorker;
    }
  }
