...
class Admin {
   ...
   *                                                  (default: READ_UNCOMMITTED)
   *
   * @returns {Promise<Array<import("../../types/kafkajs").SeekEntry>>}
   */
  async fetchTopicOffsetsByTimestamp(topic, timestamp, options = {}) {
    ...
    return new Promise((resolve, reject) => {
      this.#internalClient.listOffsets(topicPartitionOffset, options, (err, offsets) => {
        if (err) {
          reject(createKafkaJsErrorFromLibRdKafkaError(err));
        } else {
          // Convert offsets to an array of SeekEntry objects in the form of { partition: number, offset: string }
          const convertedOffsets = offsets.map(({ partition, offset }) => ({
            partition,
