...
class Admin {
  ...
  async fetchTopicOffsets(topic, options = {}) {
    ...
    let topicData;
    try {
      topicData = await this.fetchTopicMetadata({ topics: [topic] });
    } catch (err) {
    ...
    const topicPartitionOffsetsMaxTimeStamp = partitionIds.map(partitionId => ({
      topic,
      partition: partitionId,
      offset: -3
    }));
    ...
    try {
      const [latestOffsets, earliestOffsets, maxTimestampOffsets] = await Promise.all([
        this.listOffsets(topicPartitionOffsetsLatest, options),
        this.listOffsets(topicPartitionOffsetsEarliest, options),
        this.listOffsets(topicPartitionOffsetsMaxTimeStamp, options)
      ]);
      ...
      const combinedResults = partitionIds.map(partitionId => {
        ...
        const maxTimestamp = maxTimestampOffsets.find(offset => offset.partition === partitionId);

        return {
          partition: partitionId,
          offset: maxTimestamp.offset.toString(),
