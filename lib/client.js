...
const { applyAwsOAuthBearerAutowire } = require('./auto-wire-aws.js');
...
function Client(globalConf, SubClientType, topicConf, existingClient) {
  ...
  // AWS IAM OAUTHBEARER autowire — when the marker
  // `sasl.oauthbearer.metadata.authentication.type=aws_iam` is set, lazily
  // load the optional package and install its refresh callback. No-op when
  // the marker is absent (the path for >99% of clients). The autowired
  // function flows through extractFunctions below into _cb_configs.global,
  // where the existing OAUTHBEARER wrapper picks it up unchanged.
  if (!existingClient) {
