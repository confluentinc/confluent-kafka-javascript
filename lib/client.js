...
/**
 * Get the id of the cluster this client is connected to.
 *
 * The cluster id is received as part of a metadata response and is cached by
 * librdkafka. Since connecting performs a metadata request before the client
 * becomes ready, this generally returns from the cache without contacting a
 * broker.
 *
 * @param {number} timeout - Number of ms to wait for the cluster id to become
 * available. A value of 0 means the cached value is returned, or an error if it
 * is not available yet.
 * @param {RdKafka.Client~clusterIdCallback} cb - Callback to fire with the cluster id.
 */
Client.prototype.clusterId = function(timeout, cb) {
  if (!this.isConnected()) {
    if (cb) {
      return cb(new Error('Client is disconnected'));
    } else {
  ...
  this._client.clusterId(timeout, function(err, clusterId) {
    if (err) {
      if (cb) {
        cb(LibrdKafkaError.create(err));
      }
 ...
 */
  ...
  */
  ...
  /**
   * This callback is used to pass the cluster id or an error after a
   * successful query
   *
   * @callback RdKafka.Client~clusterIdCallback
   * @param {Error} err - An error, if one occurred while fetching the cluster id.
   * @param {string} clusterId - The id of the cluster the client is connected to
   */
