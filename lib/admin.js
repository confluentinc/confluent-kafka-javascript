...
AdminClient.prototype.fetchOffsets = function (options, cb) {
  console.log('Reached lib/admin.js/fetchOffsets');
  if (!this._isConnected) {
    throw new Error('Client is disconnected');
  }
  ...
  if(!options.requireStableOffsets){
  ...
  if (Array.isArray(options.topics) && options.topics.length > 0) {
    options.topics = options.topics.map(topicObj => [topicObj.topic, topicObj.partitions]);
  }
