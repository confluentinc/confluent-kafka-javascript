...
#### Configuration changes
  ...
  | **retry.retries**             | 5                                    | Total cap on the number of retries. Applies to Produce requests and to `connect()`: on a transient connection error, `connect()` retries up to this many times with exponential backoff (between `initialRetryTime` and `maxRetryTime`) before rejecting. Set to `0` to attempt `connect()` only once. Auth/config errors are not retried, and transactional/idempotent producers are not retried on `connect()`. |
