...
#### Configuration changes
  ...
  | retry.maxRetryTime            | 30000                                | maximum time to backoff a retry, in milliseconds. Also maps to `reconnect.backoff.max.ms`, capping the backoff between the underlying connection attempts during `connect()`.                                                                                                                                                                                                                              |
  | retry.initialRetryTime        | 300                                  | minimum time to backoff a retry, in milliseconds. Also maps to `reconnect.backoff.ms`, the initial backoff between the underlying connection attempts during `connect()`.                                                                                                                                                                                                                                  |
  | **retry.retries**             | 5                                    | Total cap on the number of retries. Applies to Produce requests and to `connect()`: on a transient connection error, `connect()` makes up to `retries` + 1 attempts before rejecting. Each attempt is bounded by the connection-setup timeout (`connectionTimeout` + `authenticationTimeout`) plus a small margin; the backoff between the underlying bootstrap connections is driven by librdkafka using `initialRetryTime`/`maxRetryTime`. Set to `0` to attempt `connect()` only once. As in KafkaJS, `connect()` retries by default; only terminal errors (authentication, authorization, misconfiguration, unsupported protocol/feature, fatal) are not retried, and transactional/idempotent producers are not retried on `connect()`. |
