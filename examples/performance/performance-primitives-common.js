...
function terminationRequestedCallback() {
...
function installHandlers() {
    ...
    terminateTimeout = setTimeout(terminationRequestedCallback,
                                  TERMINATE_TIMEOUT_MS);
}

function removeHandlers() {
    process.off('SIGINT', terminationRequestedCallback);
    process.off('SIGTERM', terminationRequestedCallback);
...
async function runProducer(producer, topic, batchSize, warmupMessages, totalMessageCnt, msgSize, compression, randomness, limitRPS) {
    ...
    while (totalMessageCnt == -1 || messagesDispatched < totalMessageCnt) {
        ...
        while (totalMessageCnt == -1 || messagesDispatched < totalMessageCnt) {
            ...
            if (terminationRequested || messagesNotAwaited >= maxToAwait)
                break;
        }
